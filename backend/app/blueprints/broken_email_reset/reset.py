...
class ResetBase(MethodView):
    ...
    @logout_before('reset.reset_vuln')
    def get(self):
        form = ResetForm()
...
class ResetVuln(ResetBase):
    ...
    def post(self):
        ...
        if form.validate_on_submit():
            ...
            reset_url = f"http://{current_app.config.get('HOST_IP')}/reset_problem/vuln/reset/{hash}"
...
class ResetSecurity(ResetBase):
    ...
    def post(self):
        ...
        if form.validate_on_submit():
            ...
            reset_url = f"http://{current_app.config.get('HOST_IP')}/reset_problem/secure/reset/{jwt_payload}"
...
reset_bp.add_url_rule("/vuln/", view_func=reset_vuln_page)
reset_bp.add_url_rule('/vuln/reset/', view_func=reset_vuln_wait, defaults = {"hash" : None})
reset_bp.add_url_rule("/vuln/reset/<string:hash>", view_func=reset_vuln_wait)

reset_bp.add_url_rule("/secure/", view_func=reset_sec_page)
reset_bp.add_url_rule('/secure/reset/', view_func=reset_sec_wait, defaults = {"hash" : None})
reset_bp.add_url_rule("/secure/reset/<string:hash>", view_func=reset_sec_wait)


# TODO 

# close http://127.0.0.1:5000/reset_problem/secure/reset/
# close http://127.0.0.1:5000/reset_problem/secure/reset/<hash>
# jwt exp time in config
# 